...
{
    options.AddPolicy("payments", builde =>
    {
        builde.AllowAnyOrigin().AllowAnyMethod().AllowAnyHeader();
    });
});
builder.Services.Configure<PaymentStoreSetting>(
    builder.Configuration.GetSection("PaymentDatabase"));

builder.Services.AddSingleton<PaymentService>();
builder.Services.AddMediatR(typeof(PaymentService).Assembly);
builder.Services.AddControllers()
    .AddJsonOptions(
